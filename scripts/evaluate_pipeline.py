...
def evaluate_pipeline_performance():
    ...
    if (results['pipeline_method']['total'] > 0 and 
        results['baseline_method']['total'] > 0):
        
        pipeline_acc = results['pipeline_method']['correct'] / results['pipeline_method']['total']
        baseline_acc = results['baseline_method']['correct'] / results['baseline_method']['total']
        
        improvement = ((pipeline_acc - baseline_acc) / baseline_acc) * 100 if baseline_acc > 0 else 0
...
def compare_models():
    ...
    for config in configurations:
        start_time = time.time()
        results = pipeline.detect_and_recognize(demo_image, use_pdlpr=config['use_pdlpr'])
