...
class YOLOv5Inference:
    ...
    def detect_plates(self, image_path):
        ...
        for i, det in enumerate(detections):
            ...
            conf = det["confidence"]
            print(
                f"  Plate {i + 1}: bbox=({bbox[0]}, {bbox[1]}, {bbox[2]}, {bbox[3]}), conf={conf:.2f}"
            )
    ...
    def draw_detections(self, image_path, detections, output_path=None):
        ...
        for detection in detections:
            ...
            cv2.putText(
                image, label, (x, y - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 0), 2
            )
...
def demo_detection():
    ...
    cv2.putText(
        demo_img, "ABC123", (285, 360), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 1
    )
