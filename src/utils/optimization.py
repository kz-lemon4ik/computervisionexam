...
class ImageProcessor:
    ...
    def preprocess_batch(self, images):
        ...
        for img in images:
            # Resize efficiently
            resized = cv2.resize(img, self.target_size, interpolation=self.interpolation)
...
def optimize_pipeline_for_cpu(pipeline):
    ...
    if hasattr(pipeline, 'baseline_model') and pipeline.baseline_model:
        pipeline.baseline_model = ModelOptimizer.optimize_for_cpu(pipeline.baseline_model)
    
    if hasattr(pipeline, 'recognition_model') and pipeline.recognition_model:
        pipeline.recognition_model = ModelOptimizer.optimize_for_cpu(pipeline.recognition_model)
