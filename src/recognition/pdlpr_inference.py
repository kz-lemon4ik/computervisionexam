...
class PDLPRInference:
    ...
    def __init__(self, model_path=None, device="cpu"):
        ...
        # Create model
        self.model = create_pdlpr_model(num_classes=len(self.chars), sequence_length=7)
    ...
    def decode_predictions(self, predictions):
        ...
        try:
            ...
            if hasattr(predictions, "__iter__"):
                ...
                # Single prediction
                idx_val = (
                    int(predictions) if hasattr(predictions, "__int__") else predictions
                )
                if 0 <= idx_val < len(self.chars):
                    text += self.chars[idx_val]
                else:
...
def demo_pdlpr_recognition():
    ...
    cv2.rectangle(
        demo_img, (10, 15), (118, 49), (200, 200, 200), -1
    )  # Plate background
    cv2.putText(
        demo_img, "ABC123", (20, 38), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 0), 2
    )
...
def main():
    ...
    parser.add_argument(
        "--model", type=str, default="models/pdlpr_best.pth", help="Model path"
    )
