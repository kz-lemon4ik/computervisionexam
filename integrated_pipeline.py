...
class LicensePlateRecognitionPipeline:
    ...
    def process_image(self, image_path, save_visualization=False):
        ...
        detections = self.yolo_detector.detect_plates(
            str(image_path) if isinstance(image_path, (str, Path)) else image
        )
        ...
        for i, detection in enumerate(detections):
            ...
            print(
                f"  Recognized: '{recognized_text}' (conf: {recognition_confidence:.2f})"
            )
    ...
    def _save_batch_results(self, results, output_dir):
        ...
        with open(output_dir / "summary.txt", "w", encoding="utf-8") as f:
            ...
            f.write(
                f"  Average plates per image: {total_plates / total_images:.1f}\n\n"
            )
    ...
    def get_performance_stats(self):
        ...
        if self.stats["total_images"] > 0:
            return {
                ...
                "avg_detection_time": self.stats["detection_time"]
                / self.stats["total_images"],
                "avg_recognition_time": self.stats["recognition_time"]
                / self.stats["total_images"],
                "avg_total_time": self.stats["total_time"] / self.stats["total_images"],
                "plates_per_image": self.stats["total_plates"]
                / self.stats["total_images"],
            }
        return self.stats
...
def main():
    parser = argparse.ArgumentParser(
        description="Integrated License Plate Recognition Pipeline"
    )
